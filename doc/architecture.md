...
| `use_clipboard` | `boolean` | `1` (true) | No | When true (default), yank (copy) operations place text on both the internal register and the system clipboard via `Gtk3::Clipboard`. Paste operations (`p`/`P`) read from the system clipboard when the internal register is empty. |
