# Visual Regression Tests

Automated screenshot-based regression tests for Gtk3::SourceEditor.

## Overview

Each test is a self-contained Perl macro file in `xt/visual/macros/`.  The
macro contains all the information needed for the test: the code to display,
the editor theme, language, options, a human-readable description, and the
run sub that configures the editor and captures snapshots.

The test runner (`run_visual_tests.pl`) discovers macros automatically,
launches a headless GTK editor for each one, and compares the output PNG
against a golden reference image.

## Files

```
xt/visual/
  run_visual_tests.pl     Test runner (discovers macros, compares output)
  snapshot_editor.pl      Child process (creates editor, runs macro, captures PNG)
  macros/                 Self-contained test macro files (one per test)
  golden/                 Golden reference images (generated by --init)
  output/                 Latest test output (overwritten each run)
  diffs/                  Diff images (generated on failure with --generate-diff)
```

## Running Tests

```bash
# Create/update all golden images
perl xt/visual/run_visual_tests.pl --init

# Create golden images only for tests that don't have one yet
perl xt/visual/run_visual_tests.pl --init-missing

# Run all tests against golden images
perl xt/visual/run_visual_tests.pl

# Run a single test
perl xt/visual/run_visual_tests.pl --target visual_dark_theme

# Run with diff image generation on failure
perl xt/visual/run_visual_tests.pl --generate-diff

# List all test names
perl xt/visual/run_visual_tests.pl --list

# Show GTK warnings from child processes
perl xt/visual/run_visual_tests.pl --verbose

# Pass --debug to snapshot_editor.pl for timing output
perl xt/visual/run_visual_tests.pl --debug
```

The script exits 0 if all tests pass, 1 on any failure.

## How It Works

1. `run_visual_tests.pl` scans `xt/visual/macros/` and loads every file
   via `Gtk3::SourceEditor::Macro`.  Each macro returns a hashref with
   metadata (desc, description, vim_mode, etc.) and a `run` coderef.

2. For each macro that has a `desc` field (visual tests), the runner
   spawns `snapshot_editor.pl` as a child process.  The only
   construction-time option passed is `--vim-mode` (if the macro
   requests non-default vim mode).

3. `snapshot_editor.pl` creates a `Gtk3::SourceEditor` widget in an
   off-screen window with default settings, waits for a startup delay
   (default 500ms), then runs the macro.

4. The macro's `run` sub configures everything at runtime: sets the
   theme, language, buffer content, editor options (line numbers, cursor
   line), and captures one or two snapshot PNGs via `$ctx->snapshot()`.

5. The runner compares the output PNG(s) against golden images using
   pixel-by-pixel comparison with a configurable threshold (default 1%).

## Macro Format

A visual test macro is a Perl file that returns a hashref:

```perl
my $CODE = <<'END_CODE';
...code to display in the editor...
END_CODE

{
    desc        => 'Short description shown in test output',
    description => <<'END_DESC',
    Longer description written to golden/<name>.txt during --init.
    Explains what the test checks and what to verify visually.
    END_DESC
    vim_mode    => 1,             # optional, only if non-default (0)
    run => sub {
        my ($ctx) = @_;
        $ctx->editor->set_theme('dark');
        $ctx->editor->set_language('perl');
        $ctx->buffer->set_text($CODE);
        $ctx->delay(100);
        $ctx->snapshot();          # saves <name>.png
    },
}
```

Action tests (before/after) use two labeled snapshots:

```perl
    run => sub {
        my ($ctx) = @_;
        $ctx->editor->set_theme('dark');
        $ctx->buffer->set_text($CODE);
        $ctx->delay(100);
        $ctx->snapshot('1');       # saves <name>_1.png
        $ctx->keys('dd');
        $ctx->delay(100);
        $ctx->snapshot('2');       # saves <name>_2.png
    },
```

### Macro Metadata Fields

| Field | Description |
|-------|-------------|
| `desc` | Short description (required for visual tests) |
| `description` | Long description for .txt files (optional) |
| `vim_mode` | Set to `0` to disable vim bindings (default: 1) |
| `run` | Coderef that receives `$ctx` (required) |

### Macro Context API (`$ctx`)

| Method | Description |
|--------|-------------|
| `editor()` | Returns the Gtk3::SourceEditor instance |
| `textview()` | Returns the Gtk3::SourceView widget |
| `buffer()` | Returns the Gtk3::TextBuffer |
| `mode()` | Returns current vim mode string |
| `snapshot($label?)` | Capture PNG to output dir |
| `key($name)` | Press a named GDK key (e.g. 'Return', 'Escape') |
| `type($text)` | Type each character individually |
| `keys($sequence)` | Parse escape sequences and press keys |
| `ex($command)` | Enter command mode, type command, press Enter |
| `delay($ms)` | Wait N milliseconds (processes GTK events) |
| `cursor_line()` | 0-based cursor line number |
| `cursor_col()` | 0-based cursor column |
| `line_text($n)` | Text of line $n (0-based) |
| `line_count()` | Total number of lines |
| `buffer_text()` | Full buffer contents |
| `selection_text()` | Currently selected text |
| `is_modified()` | Whether buffer has unsaved changes |
| `echo(@msg)` | Print message to STDERR |
| `call($name, @args)` | Run another loaded macro |

### Macro File Naming

Macro filenames become the test name used for golden files and `--target`:
- `macros/visual_dark_theme` creates `golden/visual_dark_theme.png`
- `macros/visual_char_selection` creates `golden/visual_char_selection_1.png` and `_2.png`

Macros without a `desc` field (e.g. `example`) are utility macros and
are not included in the test suite.

## Test Types

### Static tests (single snapshot)
Take one screenshot of the editor displaying content with specific options.
Used for themes, syntax highlighting, and editor configuration testing.

### Action tests (two snapshots)
Take a "before" screenshot, inject keystrokes via the macro, then take an
"after" screenshot.  Used to verify that editor actions (search, selection,
delete, yank/paste, etc.) produce the correct visual result.

## Debugging

### Timing editor startup

Pass `--debug` to see millisecond-precision timing from snapshot_editor.pl:

```bash
perl xt/visual/run_visual_tests.pl --debug --target visual_dark_theme
```

### Diff images

When a test fails and `--generate-diff` is passed, a diff PNG is saved to
`xt/visual/diffs/`.  The diff image shows the golden image with differing
pixels highlighted in magenta (60% blend).

### Interactive mode

Run `snapshot_editor.pl` without `--macro-run` to get an interactive GTK
window for manual testing:

```bash
perl xt/visual/snapshot_editor.pl --macro xt/visual/macros/example --theme dark
```

## Golden Images

Golden images are not tracked in git (they are environment-specific).
After any change that affects rendering (theme changes, font changes,
snapshot method changes), regenerate them with `--init`.

Each golden image has a companion `.txt` description file extracted from
the macro metadata, explaining what the test checks and what to verify
visually.
